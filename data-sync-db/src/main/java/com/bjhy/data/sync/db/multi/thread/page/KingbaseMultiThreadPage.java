...
public class KingbaseMultiThreadPage extends AbstractMultiThreadPage{
	...
	public KingbaseMultiThreadPage(SyncLogicEntity syncLogicEntity) {
	...
	private String getGeneralPageSql(int index){
		...
		String firstGeneralPageSql = "SELECT b_00_.*"
				+" FROM (SELECT rownum r_00_ ,a_00_.*"
				+" FROM ( "+fromSql+" ) a_00_ ) b_00_ where  1=1 and b_00_.r_00_ >= "+getStartPageIndex(index)+" and b_00_.r_00_ <= "+getEndPageIndex(index);
	...
	private String getHighPerformancePageSql(int index){
		String highPerformancePageColumn = syncLogicEntity.getSingleStepSyncConfig().getHighPerformancePageColumn();
		String fromSql = syncLogicEntity.getSingleStepSyncConfig().getFromSql();
		String firstHighPerformancePageSql = "SELECT b_00_."+highPerformancePageColumn
				+" FROM (SELECT rownum r_00_ ,a_00_."+highPerformancePageColumn
				+" FROM ( "+fromSql+" ) a_00_ ) b_00_ where  1=1 and b_00_.r_00_ >= "+getStartPageIndex(index)+" and b_00_.r_00_ <= "+getEndPageIndex(index);
	...
	private Integer getStartPageIndex(Integer index){
		index +=1;
		int start = ((index-1)*pageNumber)+1;
		return start;
	}
